...
///
/// # Safety
/// - config must be a valid pointer or null for defaults
/// - password must be a valid pointer with password_len bytes or null
#[no_mangle]
pub unsafe extern "C" fn secure_context_create(
    ...
    let result = panic::catch_unwind(|| {
        let rs_config = if config.is_null() {
            ...
            crate::Config::from(ptr::read(config))
        };

        let password_slice = if password.is_null() || password_len == 0 {
            None
        } else {
            Some(slice::from_raw_parts(password, password_len))
        };
...
///
/// # Safety
/// handle must be a valid pointer created by secure_context_create
#[no_mangle]
pub unsafe extern "C" fn secure_context_free(handle: *mut SecureContextHandle) -> u8 {
    if handle.is_null() {
        return FFIError::NullPointer as u8;
    }
    ...
    FFIError::Success as u8
}

/// Load identity keys into the context
///
/// # Safety
/// All pointers must be valid with 32 bytes of data
#[no_mangle]
pub unsafe extern "C" fn secure_context_load_identity(
    ...
    let result = panic::catch_unwind(|| {
        ...
        let ctx_handle = &mut *handle;
        let ctx = &mut *ctx_handle.context;

        let ed_slice = slice::from_raw_parts(ed_pub_ptr, 32);
        let x_slice = slice::from_raw_parts(x_pub_ptr, 32);
...
#[no_mangle]
pub unsafe extern "C" fn secure_session_create(
...
/// All pointers must be valid
#[no_mangle]
pub unsafe extern "C" fn secure_context_perform_handshake(
    handle: *mut SecureContextHandle,
    peer_id: *const uint8_t,
    peer_id_len: size_t,
    initiator: u8,
...
/// All pointers must be valid
#[no_mangle]
pub unsafe extern "C" fn secure_context_encrypt_message(
    handle: *mut SecureContextHandle,
    ...
    if handle.is_null() || session_id.is_null() || plaintext.is_null()
...
/// All pointers must be valid
#[no_mangle]
pub unsafe extern "C" fn secure_context_decrypt_message(
    handle: *mut SecureContextHandle,
    session_id: *const uint8_t,
    session_id_len: size_t,
    ...
    if handle.is_null() || session_id.is_null() || ciphertext.is_null()
